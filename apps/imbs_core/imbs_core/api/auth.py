...
from imbs_core.security.jwt_auth import decode_token, issue_token
...
def _auth_secret() -> str:
    return frappe.conf.get("imbs_jwt_secret") or frappe.local.conf.get("encryption_key") or "imbs-default-secret"


def _rate_limit_key(identifier: str) -> str:
    now_window = int(time.time()) // RATE_LIMIT_WINDOW_SECONDS
    return f"imbs:auth:rate:{identifier}:{now_window}"
